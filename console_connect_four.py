...
def requested_direction_count(current_row, current_col, row_movement, col_movement, matrix, player):
    ...
    for i in range(1, 4):
        ...
        if not is_valid_place(row_index_to_check, col_index_to_check):
            break
        if (matrix[row_index_to_check][col_index_to_check] == 0 or
                matrix[row_index_to_check][col_index_to_check] != player):
            break
        count += 1
    return count


def oposite_direction_count(current_row, current_col, row_movement, col_movement, matrix, player):
    count = 0
    for i in range(1, 4):
        row_index_to_check = current_row - row_movement * i
        col_index_to_check = current_col - col_movement * i
...
def is_winner(current_row, current_col, matrix, player):
    for row_movement, col_movement in direction_mapper:
        ...
        count_oposite_direction = oposite_direction_count(current_row, current_col, row_movement, col_movement, matrix, player)
        if (count_direction + count_oposite_direction) >= 4:
            return True
    return False
...
print('You are playing "CONNECT 4" game, developed by Vladimir Dyanov')
#GAME ON
print_matrix(matrix)
...
while True:
    try:
        ...
        if is_winner(current_row, current_col, matrix, player):
            print(f"Player {player} just CONNECT 4 !")
